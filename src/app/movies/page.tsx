					...
					<tbody>
						{movies.map(movie => (
							<tr key={movie.id} className="odd:bg-gray-50 hover:bg-gray-100">
								<td className="text-left w-[10%] p-1">...{movie.id.substring(movie.id.length - 6, movie.id.length)}</td>
								<td className="text-left w-[45%] p-1">{movie.title}</td>
								<td className="text-left w-[45%] p-1">
									<button
										type="button"
